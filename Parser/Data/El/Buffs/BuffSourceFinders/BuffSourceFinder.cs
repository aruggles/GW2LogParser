...
namespace Gw2LogParser.Parser.Data.El.Buffs.BuffSourceFinders
{
    internal abstract class BuffSourceFinder
    {
        ...
        private int CouldBeEssenceOfSpeed(Agent dst, long extension, ParsedLog log)
        {
        ...
        private bool CouldBeImbuedMelodies(Agent agent, long time, long extension, ParsedLog log)
        {
            if (extension == ImbuedMelodies && log.PlayerListBySpec.TryGetValue("Tempest", out List<Player> tempests))
            {
                var magAuraApplications = new HashSet<Agent>(log.CombatData.GetBuffData(5684).Where(x => x is BuffApplyEvent && Math.Abs(x.Time - time) < ParserHelper.ServerDelayConstant && x.By != agent).Select(x => x.By));

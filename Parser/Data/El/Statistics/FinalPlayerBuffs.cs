...
namespace Gw2LogParser.Parser.Data.El.Statistics
{
    ...
    public class FinalPlayerBuffs : FinalBuffs
    {
        ...
        internal static (List<Dictionary<long, FinalPlayerBuffs>>, List<Dictionary<long, FinalPlayerBuffs>>) GetBuffsForPlayers(List<Player> playerList, ParsedLog log, Agent Agent)
        {
            ...
            for (int phaseIndex = 0; phaseIndex < phases.Count; phaseIndex++)
            {
                ...
                foreach (Buff boon in boonsToTrack)
                {
                    ...
                    foreach (KeyValuePair<Player, BuffDistribution> pair in boonDistributions)
                    {
                        ...
                        {
                            hasGeneration = hasGeneration || boons.HasSrc(boon.ID, Agent);
                            double generation = boons.GetGeneration(boon.ID, Agent);
                            double overstack = boons.GetOverstack(boon.ID, Agent);
                            double wasted = boons.GetWaste(boon.ID, Agent);
                            double unknownExtension = boons.GetUnknownExtension(boon.ID, Agent);
                            double extension = boons.GetExtension(boon.ID, Agent);
                            double extended = boons.GetExtended(boon.ID, Agent);
        ...
        internal static (List<Dictionary<long, FinalPlayerBuffs>>, List<Dictionary<long, FinalPlayerBuffs>>) GetBuffsForSelf(ParsedLog log, Player player)
        {

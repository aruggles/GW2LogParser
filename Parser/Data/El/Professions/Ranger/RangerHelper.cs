...
namespace Gw2LogParser.Parser.Data.El.Professions
{
    internal static class RangerHelper
    {
        ...
        internal static readonly List<InstantCastFinder> InstantCastFinder = new List<InstantCastFinder>()
        {
            ...
            new BuffGainWithMinionsCastFinder(12633,33902,InstantCastFinders.InstantCastFinder.DefaultICD), // "Sic 'Em!"
            new BuffGainWithMinionsCastFinder(12633,56923,InstantCastFinders.InstantCastFinder.DefaultICD), // "Sic 'Em!" PvP
            new BuffGainCastFinder(12500,12543,InstantCastFinders.InstantCastFinder.DefaultICD, (evt, combatData) => Math.Abs(evt.AppliedDuration - 6000) < ServerDelayConstant), // Signet of Stone
            new BuffGainCastFinder(42470,12543,InstantCastFinders.InstantCastFinder.DefaultICD, (evt, combatData) => Math.Abs(evt.AppliedDuration - 5000) < ServerDelayConstant), // Lesser Signet of Stone
            new BuffGainCastFinder(12537,12536,InstantCastFinders.InstantCastFinder.DefaultICD), // Sharpening Stone
            new EXTHealingCastFinder(13980, 13980, InstantCastFinders.InstantCastFinder.DefaultICD), // Windborne Notes
        };
        ...
        {
            // Skills
            new BuffDamageModifier(33902, "Sic 'Em!", "40%", DamageSource.NoPets, 40.0, DamageType.Strike, DamageType.All, Source.Ranger, ByPresence, "https://wiki.guildwars2.com/images/9/9d/%22Sic_%27Em%21%22.png", 0, 115190, DamageModifierMode.PvE, (x, log) => {
                Agent src = x.From;
            ...
            new BuffDamageModifier(33902, "Sic 'Em!", "25%", DamageSource.NoPets, 25.0, DamageType.Strike, DamageType.All, Source.Ranger, ByPresence, "https://wiki.guildwars2.com/images/9/9d/%22Sic_%27Em%21%22.png", 0, 115190, DamageModifierMode.sPvPWvW, (x, log) => {
                Agent src = x.From;
                AbstractBuffEvent effectApply = log.CombatData.GetBuffData(33902).Where(y => y is BuffApplyEvent && y.To == src).LastOrDefault(y => y.Time <= x.Time);
                if (effectApply != null)
                {
            ...
            new BuffDamageModifier(33902, "Sic 'Em!", "25%", DamageSource.NoPets, 25.0, DamageType.Strike, DamageType.All, Source.Ranger, ByPresence, "https://wiki.guildwars2.com/images/9/9d/%22Sic_%27Em%21%22.png", 115190, ulong.MaxValue, DamageModifierMode.All, (x, log) => {
                Agent src = x.From;
                AbstractBuffEvent effectApply = log.CombatData.GetBuffData(33902).Where(y => y is BuffApplyEvent && y.To == src).LastOrDefault(y => y.Time <= x.Time);
                if (effectApply != null)
                {
            ...
            // Skirmishing
            new DamageLogDamageModifier("Hunter's Tactics", "10% while flanking", DamageSource.NoPets, 10.0, DamageType.Strike, DamageType.All, Source.Ranger,"https://wiki.guildwars2.com/images/b/bb/Hunter%27s_Tactics.png", (x, log) => x.IsFlanking , ByPresence, 102321, ulong.MaxValue, DamageModifierMode.All),
            ...
            new BuffDamageModifier(NumberOfBoonsID, "Bountiful Hunter", "1% per boon", DamageSource.NoPets, 1.0, DamageType.Strike, DamageType.All, Source.Ranger, ByStack, "https://wiki.guildwars2.com/images/2/25/Bountiful_Hunter.png", DamageModifierMode.All),
        };
        ...
        public static void AttachMasterToRangerGadgets(IReadOnlyList<Player> players, CombatData combatData)
        {
            ...
            HashSet<Agent> jacarandaEmbraces = ProfHelper.GetOffensiveGadgetAgents(combatData, 1286, playerAgents);
            HashSet<Agent> blackHoles = ProfHelper.GetOffensiveGadgetAgents(combatData, 31436, playerAgents);

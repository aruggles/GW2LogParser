...
namespace Gw2LogParser.Parser.Data.El.Professions
{
    internal static class WarriorHelper
    {
        ...
        internal static readonly List<InstantCastFinder> InstantCastFinder = new List<InstantCastFinder>()
        {
            ...
            new BuffGainCastFinder(45534, 45534, InstantCastFinders.InstantCastFinder.DefaultICD), // Berserker Stance
        };

        private static HashSet<Agent> GetBannerAgents(Dictionary<long, List<AbstractBuffEvent>> buffData, long id, HashSet<Agent> playerAgents)
        {
            if (buffData.TryGetValue(id, out List<AbstractBuffEvent> list))
            {
                return new HashSet<Agent>(list.Where(x => x is BuffApplyEvent && x.By.Type == Agent.AgentType.Gadget && x.By.Master == null && playerAgents.Contains(x.To.GetFinalMaster())).Select(x => x.By));
            }
        ...
        public static void AttachMasterToWarriorBanners(List<Player> players, Dictionary<long, List<AbstractBuffEvent>> buffData, Dictionary<long, List<AbstractCastEvent>> castData)
        {
            ...
            HashSet<Agent> strBanners = GetBannerAgents(buffData, 14417, playerAgents),

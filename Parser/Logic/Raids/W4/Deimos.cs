...
namespace Gw2LogParser.Parser.Logic
{
    internal class Deimos : RaidLogic
    {
        ...
        private static void MergeWithGadgets(Agent target, HashSet<ulong> gadgetAgents, List<Combat> combatData)
        {
        ...
        internal override List<AbstractBuffEvent> SpecialBuffEventProcess(Dictionary<Agent, List<AbstractBuffEvent>> buffsByDst, Dictionary<long, List<AbstractBuffEvent>> buffsById, SkillData skillData)
        {
        ...
        private static long AttackTargetSpecialParse(Combat targetable, AgentData agentData, List<Combat> combatData, HashSet<ulong> gadgetAgents)
        {
            ...
            Agent targetAgent = agentData.GetAgent(targetable.SrcAgent);
            if (targetAgent != ParserHelper._unknownAgent)
            {
                try
                {
                    ...
                    {
                        Combat structDeimosDamageEvent = combatData.FirstOrDefault(x => x.Time >= firstAware && x.IFF == ArcDPSEnums.IFF.Foe && x.DstInstid == masterInstid && x.IsStateChange == ArcDPSEnums.StateChange.None && x.IsBuffRemove == ArcDPSEnums.BuffRemove.None &&
                        ...
                        Combat armDeimosDamageEvent = combatData.FirstOrDefault(x => x.Time >= firstAware && (x.SkillID == 37980 || x.SkillID == 37982 || x.SkillID == 38046) && x.SrcAgent != 0 && x.SrcInstid != 0);
        ...
        {
            List<Agent> deimosAgents = agentData.GetNPCsByID((int)ArcDPSEnums.TargetID.Deimos);
            long offset = fightData.FightOffset;
            foreach (Agent deimos in deimosAgents)
            {
                // enter combat
                Combat enterCombat = combatData.FirstOrDefault(x => x.SrcAgent == deimos.AgentValue && x.IsStateChange == ArcDPSEnums.StateChange.EnterCombat);
                if (enterCombat != null)
                {
        ...
        internal override void EIEvtcParse(FightData fightData, AgentData agentData, List<Combat> combatData, List<Player> playerList)
        {
            ...
            // invul correction
            Combat invulApp = combatData.FirstOrDefault(x => x.DstAgent == target.Agent && x.IsBuff != 0 && x.BuffDmg == 0 && x.Value > 0 && x.SkillID == 762 && x.IsStateChange == ArcDPSEnums.StateChange.None);
            if (invulApp != null)
            {
            ...
            if (firstAware == 0)
            {
                Combat armDeimosDamageEvent = combatData.FirstOrDefault(x => x.Time >= target.LastAware && (x.SkillID == 37980 || x.SkillID == 37982 || x.SkillID == 38046) && x.SrcAgent != 0 && x.SrcInstid != 0);
                if (armDeimosDamageEvent != null)
                {
        ...
        internal override void ComputePlayerCombatReplayActors(Player p, ParsedLog log, CombatReplay replay)
        {
            // teleport zone

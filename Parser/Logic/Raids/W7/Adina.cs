...
namespace Gw2LogParser.Parser.Logic
{
    internal class Adina : RaidLogic
    {
        ...
        internal override void EIEvtcParse(FightData fightData, AgentData agentData, List<Combat> combatData, List<Player> playerList)
        {
            ...
            {
                Agent hand = agentData.GetAgent(at.DstAgent);
                Agent atAgent = agentData.GetAgent(at.SrcAgent);
                var attackables = combatData.Where(x => x.IsStateChange == ArcDPSEnums.StateChange.Targetable && x.SrcAgent == atAgent.AgentValue && x.Time <= atAgent.LastAware && x.Time >= atAgent.FirstAware).ToList();
                ...
                var posFacingHP = combatData.Where(x => x.SrcAgent == hand.AgentValue && x.Time >= hand.FirstAware && hand.LastAware >= x.Time && (x.IsStateChange == ArcDPSEnums.StateChange.Position || x.IsStateChange == ArcDPSEnums.StateChange.Rotation || x.IsStateChange == ArcDPSEnums.StateChange.MaxHealthUpdate)).ToList();
                ...
                for (int i = 0; i < attackOn.Count; i++)
                {
                    ...
                    Agent extra = agentData.AddCustomAgent(start, end, Agent.AgentType.NPC, hand.Name, hand.Prof, id, hand.Toughness, hand.Healing, hand.Condition, hand.Concentration, hand.HitboxWidth, hand.HitboxHeight);
                    foreach (Combat c in combatData)
                    {
                        if (c.Time >= extra.FirstAware && c.Time <= extra.LastAware)
                        {
                            if (c.IsStateChange.SrcIsAgent() && c.SrcAgent == hand.AgentValue)
                            {
                                c.OverrideSrcAgent(extra.AgentValue);
                            }
                            if (c.IsStateChange.DstIsAgent() && c.DstAgent == hand.AgentValue)
                            {

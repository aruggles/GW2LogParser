...
namespace Gw2LogParser.Parser.Logic
{
    internal class Qadim : RaidLogic
    {
        ...
        {
            List<Agent> pyres = agentData.GetNPCsByID((int)ArcDPSEnums.TrashID.PyreGuardian);
            // Lamps
            var lampAgents = combatData.Where(x => x.DstAgent == 14940 && x.IsStateChange == ArcDPSEnums.StateChange.MaxHealthUpdate).Select(x => agentData.GetAgent(x.SrcAgent)).Where(x => x.Type == Agent.AgentType.Gadget && x.HitboxWidth == 202).ToList();
            ...
            {
                Combat position = combatData.FirstOrDefault(x => x.SrcAgent == pyre.AgentValue && x.IsStateChange == ArcDPSEnums.StateChange.Position);
                if (position != null)
                {
        ...
        internal override long GetFightOffset(FightData fightData, AgentData agentData, List<Combat> combatData)
        {
            ...
            Combat startCast = combatData.FirstOrDefault(x => x.SkillID == 52496 && x.IsActivation.StartCasting());
            Combat sanityCheckCast = combatData.FirstOrDefault(x => (x.SkillID == 52528 || x.SkillID == 52333 || x.SkillID == 58814) && x.IsActivation.StartCasting());

...
namespace Gw2LogParser.Parser.Logic
{
    internal class River : RaidLogic
    {
        ...
        internal override void EIEvtcParse(FightData fightData, AgentData agentData, List<Combat> combatData, List<Player> playerList)
        {
            ...
            List<Agent> agentsToOverrideFirstAware = agentData.GetNPCsByID((int)ArcDPSEnums.TrashID.RiverOfSouls);
            ...
            foreach (Agent agentToOverrideFirstAware in agentsToOverrideFirstAware)
            {
                Combat firstMovement = combatData.FirstOrDefault(x => x.IsStateChange == ArcDPSEnums.StateChange.Velocity && x.SrcAgent == agentToOverrideFirstAware.AgentValue && x.DstAgent != 0);
                if (firstMovement != null)
                {
                    ...
                    {
                        if (c.SrcAgent == agentToOverrideFirstAware.AgentValue && (c.IsStateChange == ArcDPSEnums.StateChange.Position || c.IsStateChange == ArcDPSEnums.StateChange.Rotation) && c.Time <= agentToOverrideFirstAware.FirstAware)
                        {
                            sortCombatList = true;

...
namespace Gw2LogParser.Parser.Logic
{
    internal class Xera : RaidLogic
    {
        ...
        internal override long GetFightOffset(FightData fightData, AgentData agentData, List<Combat> combatData)
        {
            ...
            // enter combat
            Combat enterCombat = combatData.Find(x => x.SrcAgent == target.AgentValue && x.IsStateChange == ArcDPSEnums.StateChange.EnterCombat);
            if (enterCombat != null)
            {
        ...
        internal override void EIEvtcParse(FightData fightData, AgentData agentData, List<Combat> combatData, List<Player> playerList)
        {
            ...
            // find split
            foreach (Agent NPC in agentData.GetAgentByType(Agent.AgentType.NPC))
            {
                if (NPC.ID == 16286)
                {
                    Combat move = combatData.FirstOrDefault(x => x.IsStateChange == ArcDPSEnums.StateChange.Position && x.SrcAgent == NPC.AgentValue && x.Time >= NPC.FirstAware + 500);
                    if (move != null)
                    {
                    ...
                    var agents = new HashSet<ulong>() { NPC.AgentValue, target.AgentValue };
                    // update combat data

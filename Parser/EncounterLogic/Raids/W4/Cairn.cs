...
namespace Gw2LogParser.Parser.Logic
{
    internal class Cairn : BastionOfThePenitent
    {
        ...
        {
            Agent target = agentData.GetNPCsByID((int)ArcDPSEnums.TargetID.Cairn).FirstOrDefault();
            if (target == null)
            {
                throw new MissingKeyActorsException("Cairn not found");
            }
            // spawn protection loss -- most reliable
            Combat spawnProtectionLoss = combatData.Find(x => x.IsBuffRemove == ArcDPSEnums.BuffRemove.All && x.SrcMatchesAgent(target) && x.SkillID == 34113);
            ...
            else
            {
                ...
                Combat firstCastEnd = combatData.FirstOrDefault(x => x.EndCasting() && (x.Time - fightData.LogStart) < 2000 && x.SrcMatchesAgent(target));

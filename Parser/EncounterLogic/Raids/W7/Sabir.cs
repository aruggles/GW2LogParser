...
namespace Gw2LogParser.Parser.Logic
{
    internal class Sabir : TheKeyOfAhdashim
    {
        ...
        {
            // Find target
            Agent target = agentData.GetNPCsByID((int)ArcDPSEnums.TargetID.Sabir).FirstOrDefault();
            if (target == null)
            {
                throw new MissingKeyActorsException("Sabir not found");
            }
            Combat enterCombat = combatData.FirstOrDefault(x => x.IsStateChange == ArcDPSEnums.StateChange.EnterCombat && x.SrcMatchesAgent(target));

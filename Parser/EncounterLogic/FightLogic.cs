...
namespace Gw2LogParser.Parser.Logic
{
    public abstract class FightLogic
    {
        ...
        public IReadOnlyCollection<Agent> TargetAgents { get; protected set; }
        public IReadOnlyCollection<Agent> TrashMobAgents { get; protected set; }
        ...
        public CombatReplayMap GetCombatReplayMap(ParsedLog log)
        {
            if (_map == null)
            {
        ...
        private static void RegroupTargetsByID(int id, AgentData agentData, List<Combat> combatItems, IReadOnlyDictionary<uint, AbstractExtensionHandler> extensions)
        {
            IReadOnlyList<Agent> agents = agentData.GetNPCsByID(id);
            if (agents.Count > 1)
            {
                ...
                agentData.SwapMasters(new HashSet<Agent>(agents), newTargetAgent);
        ...
        internal virtual void ComputeFightTargets(AgentData agentData, List<Combat> combatItems, IReadOnlyDictionary<uint, AbstractExtensionHandler> extensions)
        {
        ...
        protected static List<PhaseData> GetPhasesByHealthPercent(ParsedLog log, AbstractSingleActor mainTarget, List<double> thresholds)
        {
        ...
        protected static List<PhaseData> GetPhasesByInvul(ParsedLog log, long skillID, AbstractSingleActor mainTarget, bool addSkipPhases, bool beginWithStart)
        {
            long fightDuration = log.FightData.FightEnd;
            var phases = new List<PhaseData>();
            long last = 0;
        ...
        protected void AddTargetsToPhaseAndFit(PhaseData phase, List<int> ids, ParsedLog log)
        {
            foreach (AbstractSingleActor target in Targets)
            {
        ...
        protected static void NegateDamageAgainstBarrier(CombatData combatData, List<Agent> agentItems)
        {
        ...
        internal virtual void ComputePlayerCombatReplayActors(AbstractPlayer p, ParsedLog log, CombatReplay replay)
        {
        }
        ...
        protected void SetSuccessByDeath(CombatData combatData, FightData fightData, IReadOnlyCollection<Agent> playerAgents, bool all, int idFirst, params int[] ids)
        {
        ...
        protected void SetSuccessByDeath(CombatData combatData, FightData fightData, IReadOnlyCollection<Agent> playerAgents, bool all, List<int> idsToUse)
        {
            int success = 0;
        ...
        protected static bool AtLeastOnePlayerAlive(CombatData combatData, FightData fightData, long timeToCheck, IReadOnlyCollection<Agent> playerAgents)
        {
            int playerDeadOrDCCount = 0;
            foreach (Agent playerAgent in playerAgents)
            {
        ...
        protected static void SetSuccessByCombatExit(List<AbstractSingleActor> targets, CombatData combatData, FightData fightData, IReadOnlyCollection<Agent> playerAgents)
        {
            if (targets.Count == 0)
            {
                return;
        ...
        internal virtual void CheckSuccess(CombatData combatData, AgentData agentData, FightData fightData, IReadOnlyCollection<Agent> playerAgents)
        {
        ...
        internal virtual void EIEvtcParse(ulong gw2Build, FightData fightData, AgentData agentData, List<Combat> combatData, List<AbstractSingleActor> friendlies, IReadOnlyDictionary<uint, AbstractExtensionHandler> extensions)
        {

...
namespace Gw2LogParser.Parser
{
    public class EvtcParser
    {
        ...
        public EvtcParser(ParserSettings parserSettings)
        {
        ...
        public ParsedLog ParseLog(ParserController operation, FileInfo evtc)
        {
        ...
        private void ParseSkillData(Stream stream, ParserController operation)
        {
            using (BinaryReader reader = CreateReader(stream))
            {
                ...
                for (int i = 0; i < skillCount; i++)
                {
                    ...
                    //Save
                    var skill = new Skill(skillId, name);
        ...
        private static bool IsValid(Combat combatItem)
        {
        ...
        private static void UpdateAgentData(Agent ag, long logTime, ushort instid)
        {
            if (ag.InstID == 0)
            {
                ag.SetInstid(instid);
        ...
        private void FindAgentMaster(long logTime, ushort masterInstid, ulong minionAgent)
        {
            ...
            {
                Agent minion = _agentData.GetAgent(minionAgent);
        ...
        private void CompletePlayers()
        {
            ...
            List<Agent> playerAgentList = _agentData.GetAgentByType(Agent.AgentType.Player);
            bool refresh = false;
            foreach (Agent playerAgent in playerAgentList)
            {
                ...
                {
                    Combat tst = _combatItems.Find(x => x.SrcAgent == playerAgent.AgentValue);
                    if (tst == null)
                    {
                        tst = _combatItems.Find(x => x.DstAgent == playerAgent.AgentValue);
                        if (tst == null)
                        {
                            continue;
                        }
        ...
        {
            var agentsLookup = _allAgentsList.GroupBy(x => x.AgentValue).ToDictionary(x => x.Key, x => x.ToList().First());
            ...
            foreach (Combat c in _combatItems)
            {
                ...
                {
                    if (agentsLookup.TryGetValue(c.SrcAgent, out Agent agent))
                    {
                ...
                {
                    if (agentsLookup.TryGetValue(c.DstAgent, out Agent agent))
                    {
                        UpdateAgentData(agent, c.Time, c.DstInstid);
                    }
                }
            }
            _allAgentsList.RemoveAll(x => !(x.InstID != 0 && x.LastAware - x.FirstAware >= 0 && x.FirstAware != 0 && x.LastAware != long.MaxValue) && (x.Type != Agent.AgentType.Player && x.Type != Agent.AgentType.EnemyPlayer));

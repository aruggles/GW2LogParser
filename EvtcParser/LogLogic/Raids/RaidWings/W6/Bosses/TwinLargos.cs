            ...
            case 2:
                {
                    ...
                    {
                        HealthDamageEvent? hit = log.CombatData.GetDamageTakenData(target.AgentItem).FirstOrDefault(x => x.Time >= p1.End + 5000 && pAgents.Any(x.From.IsMasterOrSelf) && x.HealthDamage > 0 && x is DirectHealthDamageEvent);
                        if (hit != null)
                        {
            ...
            case 3:
                {
                    ...
                    {
                        HealthDamageEvent? hit = log.CombatData.GetDamageTakenData(target.AgentItem).FirstOrDefault(x => x.Time >= p2.End + 5000 && pAgents.Any(x.From.IsMasterOrSelf) && x.HealthDamage > 0 && x is DirectHealthDamageEvent);
                        if (hit != null)
                        {

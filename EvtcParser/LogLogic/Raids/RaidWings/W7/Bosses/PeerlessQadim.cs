...
internal class PeerlessQadim : TheKeyOfAhdashim
{
    ...
    internal override void ComputePlayerCombatReplayActors(PlayerActor p, ParsedEvtcLog log, CombatReplay replay)
    {
        ...
        foreach (Segment seg in magmaDrop)
        {
            ...
            if (!log.CombatData.HasEffectData)
            {
                ...
                {
                    EIData.Color magmaColor = magmaCounter == 0 ? Colors.Yellow : Colors.LightOrange;
    ...
    internal override void ComputeEnvironmentCombatReplayDecorations(ParsedEvtcLog log, CombatReplayDecorationContainer environmentDecorations)
    {
        ...
        if (log.CombatData.TryGetEffectEventsByGUID(EffectGUIDs.QadimPeerlessShowerOfChaosAoE, out var whiteOrbAoEs))
        {
            ...
            foreach (EffectEvent effect in whiteOrbAoEs)
            {
                ...
                EIData.Color color = Colors.LightGrey;
    ...
    private static void AddTetherDecorations(ParsedEvtcLog log, SingleActor actor, CombatReplay replay, long buffID, EIData.Color color, double opacity)
    {

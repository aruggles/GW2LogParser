    ...
    protected LogLogic(int triggerID)
    {
        ...
        MechanicList = [
            new MechanicGroup([
                new MechanicGroup(
                    [
                        new PlayerStatusMechanic<DeadEvent>(Mech_PlayerDead, new (Symbols.X, Colors.Black), new("Dead"), Sev0, (log, a) => log.CombatData.GetDeadEvents(a))
                            .UsingNoShowOnTable(),
                        new PlayerStatusMechanic<DownEvent>(Mech_PlayerDowned, new (Symbols.Cross, Colors.Red),  new("Downed"), Sev0, (log, a) => log.CombatData.GetDownEvents(a))
                            .UsingNoShowOnTable(),
                        new PlayerStatusMechanic<AliveEvent>(Mech_PlayerUp, new (Symbols.Cross, Colors.Green), new ("Got up"), Sev1, (log, a) => log.CombatData.GetAliveEvents(a))
                            .UsingNoShowOnTable(),
                    ]
                ),
                new PlayerCastStartMechanic(SkillIDs.Resurrect, Mech_PlayerRes, new (Symbols.CrossOpen,Colors.Teal), new("Res"), Sev2)
                    .UsingNoShowOnTable(),
                new MechanicGroup(
                    [
                        new PlayerStatusMechanic<DespawnEvent>(Mech_PlayerDC, new (Symbols.X, Colors.LightGrey), new("DC"), Sev4, (log, a) => log.CombatData.GetDespawnEvents(a))
                            .UsingNoShowOnTable(),
                        new PlayerStatusMechanic<SpawnEvent>(Mech_PlayerSpawn, new (Symbols.Cross, Colors.LightBlue), new("Resp"), Sev4, (log, a) => log.CombatData.GetSpawnEvents(a))
                            .UsingNoShowOnTable()
                    ]
                ),
                new MechanicGroup(
